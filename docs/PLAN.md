...
- [x] **P6.2** (2026-09-08: ECR repo `pb-runner` created by `terraform apply -target=module.ecr` (1 add, 0 destroy); first arm64 image built on the dev box with `docker buildx --platform linux/arm64` from master 02df1a9 (65 min under QEMU, 16.8 MB layer) and pushed as `pb-runner:8-v8.1.0-arm64`, `--help` verified under emulation. 2026-09-08 later: replaced by `.github/workflows/image-build.yml` -- manual dispatch, native `ubuntu-24.04-arm` runner (7 min vs 65 under QEMU, free on a public repo), OIDC role `pb-runner-gh-build` in the pbtb-rust dev env, ECR lifecycle `keep_last_images = 20`. Each build pushes `src-<hash>` + `<git sha>` and re-points the moving version-line tag `v810` (D23), so a rollout is a build plus a bot restart. CodeBuild was never created.) ECR repo + build pipeline (user triggers builds).
...
- [ ] **P6.5** (2026-09-08: steps 1-5 of RUNBOOK "pb-runner runtime" done -- ECR repo + arm64 image, lambda-deploy 34179407866 (the `8rs`-aware binary live first), telebot-build 34184699943 after the bookworm fix (PR #36, `:latest` = d6a2139), scoped apply of `module.passivbot_task["8rs"]` + lambda + telebot base-env (2 added, 2 changed, 0 destroyed; task def `scalable-cluster-dev-passivbot-v8-rs:1`, memory 96 placeholder), telebot-deploy 34185540023 (`telebot-up`, table carries `8rs=...-v8-rs:1`). tfvars change in PR https://github.com/iengai/pbtb-rust/pull/37, unmerged -- state has `8rs`, `main` does not. Remaining: step 6 on Telegram (`/runtime <bot_id> rs`, Stop, Run) is the user's, then measure RSS and lower the 96 MiB placeholder.) Roll the `8rs` line out to dev: table entry, telebot/lambda deploy, first bot moved to the Rust runtime.
