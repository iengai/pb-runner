...
- [x] **P6.2** (2026-09-08: ECR repo `pb-runner` created by `terraform apply -target=module.ecr` (1 add, 0 destroy); first arm64 image built on the dev box with `docker buildx --platform linux/arm64` from master 02df1a9 (65 min under QEMU, 16.8 MB layer) and pushed as `pb-runner:8-v8.1.0-arm64`, `--help` verified under emulation. `deploy/buildspec.yml` remains the CodeBuild path for later builds; the project itself is not created yet.) ECR repo + build pipeline (reuse pbtb-rust CodeBuild pattern; user triggers builds).
