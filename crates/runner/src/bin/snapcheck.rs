...
/// Parse recorded JSON with correctly-rounded floats (`str::parse::<f64>`).
/// serde_json's default float parser is best-effort and lands one ulp off
/// for some literals (D8), which would show up as spurious EMA mismatches.
fn parse_exact(text: &str) -> Result<Value> {
    struct P<'a> {
        b: &'a [u8],
        i: usize,
    }
